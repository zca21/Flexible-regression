...
The most **general form** with the error distribution $f(y;\eta,\phi)$ of the response $Y$ which might not normal can be represented below (note is **conditional** on the random effects).
...
For $h^{k}(Z)=f(V,Z)$ k is the index representing the level of V, meaning the smooth function of Z depends on V. Also sometimes call the intercept $\alpha$.
...
-   May want to use RE when covariate scan be viewed as having a **probability distribution** assocaited with it
...
-   Talking about relationships in GAMs: $h^{(x)}(Z_{1})$ indicates relationship between $Y$ and $Z_{1}$ is non-linear and that re;relationships depends on the level of \$X\$. $g(Z_{2},Z_{3})$ (defined by a joint function) indicates that the relationship between $Z_{2}$ and $Y$ is non-linear and depends on the value of \$Z\_{3}\$, similarly for $Z_{3}$.
